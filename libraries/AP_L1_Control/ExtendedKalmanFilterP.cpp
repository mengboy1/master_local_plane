#include "ExtendedKalmanFilterP.h"
...
float ExtendedKalmanFilterP::measurementpredandjacobian(VectorN<float,EKF_N> &A)
{
...
float ExtendedKalmanFilterP::prediction() {
    _z = X[0] * EXP(- (powf(X[2], 2) + powf(X[3], 2)) / powf(X[1], 2));
    return _z;
}

void ExtendedKalmanFilterP::reset(const VectorN<float,EKF_N> &x, const MatrixN<float,EKF_N> &p, const MatrixN<float,EKF_N> q, float r)
{
    P = p;
    X = x;
    Q = q;
...
void ExtendedKalmanFilterP::update(float z, float Vx, float Vy)
{
