...
#pragma GCC diagnostic pop
    ...
    scaler = constrain_value(scaler, 0.5f, 2.0f);
    ...
    float t_step, float t_hori, float I_moment, float k_aileron, float k_roll_damping, float c_lp, int extend)
{
    ...
    _t_hori = t_hori;
    _t_step = t_step;
	//gcs().send_text(MAV_SEVERITY_INFO, "action_paths: _t_step %f",(float)_t_step);
    ...
    float t_step, float t_hori, float I_moment, float k_aileron, float k_roll_damping, float c_lp, int j0, int j1)
{
    ...
    float t = dt;
	//gcs().send_text(MAV_SEVERITY_INFO, "generate_action: theta_rate %d",(float)theta_rate);
    ...
    {	
        for (int i = 0; i < rate_x; i++)
        {
            ...
            theta += theta_rate * dt;
            psi -= dt * (g * tan((theta * M_PI) / 180.0f) / v0);
            px += dt * v0 * sin(psi);
            py += dt * v0 * cos(psi);
            t += dt;
        }
		...
		gcs().send_text(MAV_SEVERITY_INFO, "theta %f",double(theta));
...
void PomdpSolver::init_step(int max_loops, int n, float x0, float p0, float q0, float r0,
    float weights[4], bool max_lift)
{
	...
	//gcs().send_text(MAV_SEVERITY_INFO, "Tuning: _therm_x");
	//gcs().send_text(MAV_SEVERITY_INFO, "init_step: _therm_y %f",(float)_therm_y);
...
void PomdpSolver::inner_loop()
{
	...
	if(_p0 > 0.6f && _p0 < 0.63f)
	{
        kout = 4.0f;
	}
	else 
		{
		kout += 4.0f + fabs(_action_path_theta[_m][_j]/15.0f);
...
int PomdpSolver::update()
{
	...
	//gcs().send_text(MAV_SEVERITY_INFO, "Tuning: _solve_time %d",_solve_time);
    if (_generate_actions)
    {
        ...
        //gcs().send_text(MAV_SEVERITY_INFO, "Tuning: j1 %d",j1);
    ...
    while (_loop < _max_loops)
    {
        ...
        if (_j >= _k)
        {
            ...
            {
                _Q[_m] += (_weights[0] * 0.5f
                           + _weights[1] * 0.5f
                           + _weights[2] * 0.5f
                           + _weights[3] * 0.5f * 1.0 / _n);
            }
            ...
            if (_i >= _n)
            {
                ...
                if (_err && _Q[_m] < _Q[_best_action])
                {
                    _best_action = _m;
					gcs().send_text(MAV_SEVERITY_INFO, "_best_action %d",int(_best_action));
                }
	...
	gcs().send_text(MAV_SEVERITY_INFO, "Soaring: samp_count %f", (double)samp_count);
