...
void AP_L1_Control::update_waypoint(const struct Location &prev_WP, const struct Location &next_WP, float dist_min)
{
	...
	gcs().send_text(MAV_SEVERITY_INFO, "Tuning: x,y %.2f %.2f",(float)A_air.x,(float)A_air.y);
...
//    hal.console->println(S2ctoav.z);
	...
	if (best_action == 0)
      ...
      const float K_L1 = 4.0f * _L1_damping * _L1_damping;
/*
	  AP::logger().Write("L13d", "TimeUS,Err", "Qf",
                                               AP_HAL::micros64(),
                                               (double)_crosstrack_error
                                               );
                                               */
       gcs().send_text(MAV_SEVERITY_INFO, "Tuning: crosstrack_3d %.2f",(float)_crosstrack_error);
       AP::logger().Write("L13d", "TimeUS,Err,k1", "Qff",
                                               AP_HAL::micros64(),
                                               (double)_crosstrack_error,
                                               (double)K_L1
                                               );
