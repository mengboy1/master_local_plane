...
void AP_L1_Control::update_waypoint(const struct Location &prev_WP, const struct Location &next_WP, float dist_min)
{
	...
	Vector3f velav=Vector3f(0.0f,0.0f,0.0f);
    ...
    float velal = MAX(velalv.length(), 1.0f);//和地速值差不多
    Vector2f velalvxz(velav.x,velav.z);
	//Vector2f velalvyz(velav.y,velav.z);
	float velalxz = MAX(velalvxz.length(), 1.0f);//xoz平面
	//float velalyz = MAX(velalvyz.length(), 1.0f);//yoz平面
	//gcs().send_text(MAV_SEVERITY_INFO, "Tuning: velar,xz,yz %.2f %.2f %.2f",(float)velal,(float)velalxz,(float)velalyz);
    ...
    Vector3f AB = prev_WP.get_distance_NED(next_WP);//three vector
	Vector2f AB_l = Vector2f(AB.x,AB.y);//lateral projection of prev_wp and next_wp
	Vector2f AB_xz = Vector2f(AB.x,AB.z);
	//Vector2f AB_yz = Vector2f(AB.y,AB.z);
    float AB_length = AB.length();

	const int32_t D = next_WP.alt - _current_loc.alt;
     //const int32_t D =AB.z;
	_current_loc.alt= _current_loc.alt + D ;
	//float AB_xz_length=AB_xz.length();//xoz平面
	//float AB_yz_length=AB_yz.length();
    //gcs().send_text(MAV_SEVERITY_INFO, "Tuning: AB,D,alt %.2f %.2f %.2f",(float)AB_length,(float)D,(float)_current_loc.alt);
	//gcs().send_text(MAV_SEVERITY_INFO, "Tuning: velal,x,y,AB %.2f %.2f %.2f %.2f",(float)velal,(float)AB.x,(float)AB.y,(float)AB_length);
    // Check for AB zero length and track directly to the destination
    // if too small
    if (AB.length() < 1.0e-6f) {
        //AB = _current_loc.get_distance_NE(next_WP);
         AB = prev_WP.get_distance_NED(next_WP);//three vector
        if (AB.length() < 1.0e-6f) {
            AB = Vector3f(cosf(get_yaw()), sinf(get_yaw()),0.0f);
        }
    }
    AB.normalize();
	AB_l.normalize();
    ...
    const Vector3f A_air = prev_WP.get_distance_NED(_current_loc);
    Vector2f A_air_l=Vector2f(A_air.x,A_air.y);//lateral prohection of aircraft to prev_next
    Vector2f A_air_xz=Vector2f(A_air.x,A_air.z);//xoz
    //Vector2f A_air_yz=Vector2f(A_air.x,A_air.z);//xoz
	//gcs().send_text(MAV_SEVERITY_INFO, "Tuning: A_air,xz,yz %.2f %.2f %.2f",(float)A_air_l.length(),(float)A_air_xz.length(),(float)A_air_yz.length());
    // calculate distance to target track, for reporting
    //_crosstrack_error = (A_air) % AB;
    _crosstrack_error = (A_air_l) % AB_l;
	float _crosstrack_z = (A_air_xz) % AB_xz;//vertical plane
	//gcs().send_text(MAV_SEVERITY_INFO, "Tuning: velar,groSped,ab3,ab2 %.2f %.2f %.2f %.2f",(float)velav.length(),(float)groundSpeed,(float)AB_length,(float)AB_l.length());
	...
	AP::logger().Write("L1", "TimeUS,L1,croserr_l,AB,AB_X,AB_Y,AB_Z,WP_A_dist,alongTrackDist", "Qffffffff",
                                               AP_HAL::micros64(),
                                               (double)_L1_dist,
    ...
    if (WP_A_dist > _L1_dist && alongTrackDist/MAX(WP_A_dist, 1.0f) < -0.7071f)
    {
        ...
        //Vector2f A_air_unit = (A_air).normalized(); // Unit vector from WP A to aircraft
         //Vector2f A_air_unit = (A_air_l).normalized(); // Unit vector from WP A to aircraft
         Vector3f A_air_unit = (A_air).normalized();
		//xtrackVel = _groundspeed_vector % (-A_air_unit); // Velocity across line
        //ltrackVel = _groundspeed_vector * (-A_air_unit); // Velocity along line
        //xtrackVel = velalv % (-A_air_unit); // Velocity across line
        //ltrackVel = velalv * (-A_air_unit); // Velocity along line
        Vector3f xtrackVelvector=velav % (-A_air_unit);
        xtrackVel = xtrackVelvector.length(); // Velocity across line
        ltrackVel = velav * (-A_air_unit); // Velocity along line
        Nu = atan2f(xtrackVel,ltrackVel);
		//z direction
		float xtrackVelz = velalvxz % AB_xz; // Velocity across line
        float ltrackVelz = velalvxz * AB_xz; // Velocity along track
		Nuxz=atan2(xtrackVelz, ltrackVelz);
        _nav_bearing = atan2f(-A_air_unit.y , -A_air_unit.x); // bearing (radians) from AC to L1 point
        gcs().send_text(MAV_SEVERITY_INFO, "Tuning: Anav %.2f",(float)_nav_bearing);
        ...
        const Vector3f B_air = next_WP.get_distance_NED(_current_loc);
		Vector3f B_air_unit = (B_air).normalized(); // Unit vector from WP B to aircraft
		Vector3f xtrackVelvectorB=velav % (-B_air_unit); // Velocity across line
		xtrackVel = xtrackVelvectorB.length(); // Velocity across line
        ltrackVel = velav * (-B_air_unit); // Velocity along line
        Nu = atan2f(xtrackVel,ltrackVel);
		//z direction
		float xtrackVelz = velalvxz % AB_xz; // Velocity across line
        float ltrackVelz = velalvxz * AB_xz; // Velocity along track
        ...
        gcs().send_text(MAV_SEVERITY_INFO, "Tuning: Bnav %.2f",(float)_nav_bearing);
    } else { //Calc Nu to fly along AB line

        //Calculate Nu2 angle (angle of velocity vector relative to line connecting waypoints)
        //xtrackVel = _groundspeed_vector % AB; // Velocity cross track
        //ltrackVel = _groundspeed_vector * AB; // Velocity along track
        xtrackVel = _groundspeed_vector % AB_l; // Velocity cross track
        ltrackVel = _groundspeed_vector * AB_l; // Velocity along track
        
        float xtrackVelz = velalvxz % AB_xz; // Velocity across line of  longitudinal direction 
        float ltrackVelz = velalvxz * AB_xz; // Velocity along track of  longitudinal direction 
        float Nu2xz = atan2f(xtrackVelz, ltrackVelz); 
		...
		//z ordination
		float sine_Nu1xz = _crosstrack_z/MAX(_L1_dist, 0.1f);
        //Limit sine of Nu1 to provide a controlled track capture angle of 45 deg
        sine_Nu1xz = constrain_float(sine_Nu1xz, -0.7071f, 0.7071f);
        float Nu1xz = asinf(sine_Nu1xz);
        ...
        //gcs().send_text(MAV_SEVERITY_INFO, "Tuning: cross,Nuxz,Nu %.2f %.2f %.2f",(float)_crosstrack_z,(float)Nuxz,(float)Nu);
    }
	...
	//z direction
	Nuxz = constrain_float(Nu, -1.5708f, +1.5708f);
   // _latAccDem = K_L1 * groundSpeed * groundSpeed / _L1_dist * sinf(Nu);
   float _latAccDemxy = K_L1 * velal * velal / _L1_dist * sinf(Nu);//横向平面
   float _latAccDemxz = K_L1 * velalxz * velalxz / _L1_dist * sinf(Nuxz);//纵向平面
   _latAccDem = K_L1 * velal * velal / _L1_dist * sinf(Nu);
    //_latAccDem=norm(_latAccDemxy, _latAccDemxz);
   // _latAccDem=MAX(fabs(_latAccDemxy),fabs(_latAccDemxz));

    //gcs().send_text(MAV_SEVERITY_INFO, "Tuning: AB,D,alt,axz %.2f %.2f %.2f %.2f",(float)AB_length,(float)D,(float)_current_loc.alt,(float)_latAccDemxz);
	gcs().send_text(MAV_SEVERITY_INFO, "Tuning: cross,axz,a %.2f %.2f %.2f",(float)_crosstrack_z,(float)_latAccDemxz,(float)_latAccDemxy);
...
//    hal.console->println(S2ctoav.z);
    ...
    // Calculate guidance gains used by PD loop (used during circle tracking)
      const float omega = (6.2832f / _L1_period);
      const float Kx = omega * omega;
      const float Kv = 1.0f * _L1_damping * omega;
      // Calculate L1 gain required for specified damping (used during waypoint capture)
      const float K_L1 = 4.0f * _L1_damping * _L1_damping;
